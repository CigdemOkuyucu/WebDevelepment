...
import { deletePerson, loadPeople } from "./overview.api"

export interface OverviewProps{
    //Storage will go to the API
    //storage: Map<number,PersonEntry>
    backToHome: () => void
    //These will be allrepolaced by API calls
    //Exercise 3
...
export class Overview extends React.Component<OverviewProps,OverviewState>{
    ...
    loadOverview(){
        if (this.state.overviewLoader.kind == "loading"){
            loadPeople() //now people is correctly stored inside the state after the promise resolves.
            .then(people => 
    ...
    //special method after something happens
                                    ...
                                    Age: {person.age } 
                                    {
                                        this.state.mode == "edit" ?
                                            <button
                                                ...
                                                //this can be also added to the map.
                                                disabled = {this.state.deleteLoader == "loading"}
                                                onClick={ _ => {
                                                    this.setState(this.state.updateDeleteLoader("loading"), () => {
                                                        deletePerson(person.id)
                                                        .then(_ => {
                                                            this.setState(this.state.updateDeleteLoader("loaded"), () => {
